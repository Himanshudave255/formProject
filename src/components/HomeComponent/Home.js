...
class Home extends Component {
	constructor(props) {
		...
		this.state = {
			...
			aboutUs: props.loginStatus
				? 'For more information Contact: Himanshu 9711046797'
			...
			addDialogForm: {
				...
				name: {
					...
					status: false,
					validators: { required: true, minLength: 2, email: false, maxLength: 20, custom: false },
					variant: 'standard',
				},
				//2nd object
				email: {
		...
		if (currentLoginUser && currentLoginUser.role === 'Employee') {
			newDataArray = dataArray.filter((res, index) => {
				if (res.email === currentLoginUser.email) {
					res.Issues = (
						<Link onClick={this.routeToIssue.bind(this, index)} to={`/home/${res.userId}`}>
							Click here
						</Link>
					);
					return res;
		...
		return (
			<div>
				{this.props.loginStatusFromRedux ? (
					<div>
						<Link onClick={this.logout} className="logout-user" to="/">
							Logout
						</Link>
				...
				JSON.parse(localStorage.getItem('currentLogin')).role === 'Admin' ? (
					<div>
						<Button variant="contained" onClick={this.addRow} style={useStyles.button}>
							ADD
						</Button>
						<Button
							variant="contained"
						...
						<Button
							...
							color="secondary"
							disabled={this.state.deleteButtonDisable}
							style={useStyles.button}
						>
							{this.state.deletedRowsArray.length > 1 ? 'DELETE MULTIPLE' : 'DELETE'}
						</Button>
					</div>
				) : (
				...
				<DialogComponent
					...
					content={
						<RUCforms
							submit={this.state.triggerSubmit}
							submitButton={false}
							custom={customValidation}
							formOutput={this.formData}
							form={this.state.addDialogForm}
						/>
					}
